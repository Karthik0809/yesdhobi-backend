...
const statusEnum = z.enum(['PENDING_PICKUP', 'ASSIGNED', 'PICKED_UP', 'IN_LAUNDRY', 'WASHING', 'IRONING', 'QUALITY_CHECK', 'READY', 'OUT_FOR_DELIVERY', 'DELIVERED', 'CANCELLED']);
...
adminOrdersRouter.post(
  '/:id/status',
  asyncHandler(async (req, res) => {
    const { status, reason, force } = parseBody(z.object({ status: statusEnum, reason: z.string().optional(), force: z.boolean().default(true) }), req.body);
    const order = await loadOrder(req.params.id!);
